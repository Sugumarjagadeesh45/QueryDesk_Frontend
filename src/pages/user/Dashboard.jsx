...
import { ArrowRight, Plus, Inbox, Clock, Activity, CheckCircle2, MessageSquareOff, LoaderCircle } from 'lucide-react';
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-6 md:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-slate-900 tracking-tight">Welcome, {user?.name} 👋</h1>
          <p className="text-slate-500 text-sm mt-1">Here's an overview of your queries.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
          <StatCard label="Total" value={stats.total} icon={Inbox} colorClass="bg-blue-50 text-blue-600" />
          <StatCard label="Pending" value={stats.pending} icon={Clock} colorClass="bg-amber-50 text-amber-600" />
          <StatCard label="In Progress" value={stats.inProgress} icon={Activity} colorClass="bg-indigo-50 text-indigo-600" />
          <StatCard label="Resolved" value={stats.resolved} icon={CheckCircle2} colorClass="bg-emerald-50 text-emerald-600" />
        </div>
      )}
      ...
      <Card className="overflow-hidden">
        <div className="flex items-center justify-between px-5 md:px-6 py-4 border-b border-slate-200 bg-white">
          ...
          <Link to="/user/queries" className="text-sm font-medium text-blue-600 hover:text-blue-700 flex items-center gap-1 group">
            View all
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description="Submit your first query and our support team will help you out."
                ...
                <tbody className="divide-y divide-slate-200 bg-white">
                  {queries.slice(0, 5).map((q) => (
                    <tr key={q._id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="px-6 py-4 text-slate-700 max-w-xs truncate">{q.subject}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
